...
public class AnalysisService {
    ...
    private String buildJobDescriptionPrompt(
        ...
        return """
                ...
                이미지 중 무엇이든 공고의 원문 텍스트를 확보한다.

                # 입력 종류별 처리
                - URL: 입력 URL에서 이미 크롤링된 텍스트를 기준으로 공고 본문을 확인한다.
                - 텍스트: 입력 텍스트를 그대로 사용한다.
                - 이미지: 첨부 이미지와 이미지 OCR 텍스트를 함께 참고해 공고 내용을 읽는다.
                - 여러 입력이 함께 제공되면 같은 공고 자료로 보고 중복은 제거하되,
                  한쪽에만 있는 채용 핵심 정보는 누락하지 않는다.
                - URL 크롤링 텍스트가 비어 있어도 텍스트/OCR/첨부 이미지 중 하나에서 공고 본문을 읽을 수 있으면 성공으로 처리한다.

                # 규칙
                - raw_text에는 공고 본문을 있는 그대로 확보한다. 이 단계에서는 요건을 추출·평가하지 마라.
                - 원문의 줄·항목 구조를 최대한 보존한다.
                - raw_text를 한 문장 요약이나 콤마로 압축하지 마라.
                ...
                # 불러오기 실패 판정
                아래 경우 success: false:
                - URL 크롤링 텍스트, 입력 텍스트, 이미지 OCR 텍스트, 첨부 이미지 모두에서 채용공고 본문을 읽을 수 없음
                - 제공된 이미지만으로 판단해야 하는데 이미지가 흐릿해 글자를 읽을 수 없음
                - 입력이 채용 공고가 아님
                - 개발자 채용 공고가 아님
