...
public class AnalysisService {
    ...
    private String buildJobDescriptionPrompt(
        ...
        return """
                ...
                # 규칙
                - raw_text에는 URL 크롤링 결과와 이미지 OCR 결과를 별도 출처로 나누지 말고 하나의 채용공고 문서로 합성한 최종 원문을 담는다.
                - raw_text를 한 문장이나 콤마로 이어진 요약문으로 압축하지 마라.
                - raw_text는 화면에서 원본 공고 영역에 그대로 보여줄 값이므로 줄바꿈과 항목 구조를 반드시 유지한다.
                - URL과 이미지는 서로 보완하는 하나의 공고 자료다. URL에서 얻은 모집요강과 이미지에서 얻은 직무 상세를 섹션별로 합쳐 하나의 결과만 만든다.
                - "URL 내용"과 "이미지 내용"처럼 출처별 제목을 만들지 마라.
                - URL 크롤링 결과에 없는 업무내용, 자격요건, 우대사항, 보안 솔루션/클라우드/ISMS-P/ISO 27001 관련 항목이 이미지 OCR에 있으면 해당 섹션에 자연스럽게 병합한다.
                - 이미지 OCR에서 읽은 직무 상세가 raw_text에 빠지면 실패한 응답이다.
                ...
                - 한 입력에만 있는 채용 핵심 정보는 누락하지 말고 알맞은 섹션에 병합한다.
                ...
                - raw_text는 회사명, 포지션, 포지션 소개, 모집요강, 업무내용, 자격요건, 우대사항, 접수기간, 근무조건 중심으로 정리한다.
                - raw_text에는 아래 형식을 사용한다.
                  # 회사명 - 포지션명
                  ## 포지션 소개
                  - URL과 이미지 전체를 분석해 도출한 조직/포지션 소개
                  ## 모집요강
                  - 모집분야:
                  - 모집인원:
                  - 고용형태:
                  ...
                  - 근무지:
                  ## 업무내용
                  - URL과 이미지 전체를 분석해 도출한 업무내용 항목
                  ## 자격요건
                  - 경력:
                  - 학력:
                  - URL과 이미지 전체를 분석해 도출한 자격요건 항목
                  ## 우대조건
                  - URL과 이미지 전체를 분석해 도출한 우대조건 항목
                  ## 접수기간
                  - 시작일:
                  - 마감일:
                  ## 참고 정보
